class User < ApplicationRecord
  ...
  devise :database_authenticatable, :registerable,
         ...
         with_options presence: true, do
          ...
          validates :email, presence: true, unique: true, format: { with: (/@.+/) }
          validates :encrypted_password, presence: true, length: {minimum: 6 }, format:{ with: (/\A(?=.*?[a-z])(?=.*?[\d])[a-z\d]+\z/i), message: '６文字以上の半角英数字' }
